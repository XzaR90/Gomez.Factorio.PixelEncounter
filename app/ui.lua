...
local function build_menu_items(global_player, menu_frame)
    ...
    global_player.elements.menu_buttons = {};
    global_player.elements.menu_buttons["character"] = menu_btn_character
    global_player.elements.menu_buttons["modifiers"] = menu_btn_modifiers
end
...
local function build_content_frames(global_player, grid_frame)
    ...
    global_player.elements.content_frames = {}
    global_player.elements.content_frames["character"] = character_content_frame
    global_player.elements.content_frames["modifiers"] = modifiers_content_frame
end

local function toggleMenuItem(global_player, menuName)
    for k, v in pairs(global_player.elements.menu_buttons) do
        v.enabled = true
        global_player.elements.content_frames[k].visible = false
        if menuName == k then
            v.enabled = false
            global_player.elements.content_frames[k].visible = true
        end
    end
end
...
function UI.build_interface(player)
    ...
    global_player.elements.grid_frame = grid_frame
    
    local menu_frame = grid_frame.add{type="frame", name="pe_menu_frame", direction="vertical"}
    global_player.elements.menu_frame = menu_frame
    ...
    ModifiersUI.build(player,global_player)
end

function UI.toggle_interface(player)
    
    local global_player = PlayerUtil.get_global_player(player);
    local main_frame = global_player.elements.main_frame
